...
<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card">
        <div class="card-header row justify-content-between align-items-center">
            <h4 class="col-md-3"><span class="badge bg-label-info me-1">Orders to deliver</span></h4>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        function fetchOrders() {
            fetch('{{ route("customers.deliveryUpdate") }}')
                .then(response => {
                ...
                .then(data => {
                    ...
                    if (Object.keys(userByReference).length > 0) {
                        Object.keys(userByReference).forEach((referenceNo, index) => {
                            ...
                            <td>
                                <span class="badge bg-${user.payment_condition === 'paid' ? 'success' : (user.payment_proof && user.payment_type === 'G-cash' && (user.order_retrieval === 'delivery' || user.order_retrieval === 'pickup') ? 'warning' : 'danger')} me-1">${user.payment_condition === 'paid' ? 'Paid' : (user.payment_proof && user.payment_type === 'G-cash' && (user.order_retrieval === 'delivery' || user.order_retrieval === 'pickup') ? 'Pending' : 'Unpaid')}</span>
                            </td>
                            <td>
                                <a class="bx bx-message-alt me-1 details-button" href="#" data-bs-toggle="modal" data-bs-target="#messagesModal" data-reference-no="${user.referenceNo}" id="courierId"></a>
                            </td>
                            <td>
                                ${user.delivery_date
                                    ? `<span class="badge bg-success">${new Date(user.delivery_date).toLocaleString('en-PH', { timeZone: 'Asia/Manila', weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })} ${new Date(user.delivery_date).toLocaleTimeString('en-US', { hour: 'numeric', minute: 'numeric', hour12: true })}</span>`
                                    : `<span class="badge bg-danger">Not scheduled yet</span>`}
                            </td>
                            `;
